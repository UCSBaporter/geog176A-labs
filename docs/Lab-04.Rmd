...
MS_river = dam_sf %>% group_by(STATE) %>%  filter(HAZARD == "H") %>% slice_max( NID_STORAGE, n=1) %>% ungroup()%>%  point_in_polygon3(hex_grid, 'id') %>% plot_pip('Hazard Dam') + gghighlight(n > (mean(n) + sd(n))) +
  labs(title = "Number of Hazard Dams:")  
